...
export function FinanceTab() {
  ...
  const periodEntries = useMemo(
    () => entries.filter((e) => inPeriod(e.month) && matchClass(e.class_type)),
    [entries, view, month, year, cls],
  );

  const tuitionEntries = useMemo(
    () => periodEntries.filter((e) => e.kind === "thu" && e.income_type === "hoc_phi"),
    [periodEntries],
  );
  ...
  const autoTuitionRows = useMemo(() => {
    return students.filter((s) => {
      ...
      const manual = entries.some(
        (e) => e.income_type === "hoc_phi" && e.student_name === s.name && (e.course_label ?? "") === label,
      );
  ...
  const tuitionIncome =
    sum(paidRows as any) + sum(tuitionEntries) + autoTuitionRows.reduce((a, s) => a + Number(s.tuition), 0);
  ...
  const byMonth = useMemo(() => {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      const thu =
        payments
          .filter(
            (p) => p.month.slice(0, 7) === key && (cls === "Tất cả" || stuMap.get(p.student_id)?.class_type === cls),
          )
          .reduce((a, b) => a + Number(b.amount), 0) +
        entries
          .filter((e) => e.month.slice(0, 7) === key && e.kind === "thu" && matchClass(e.class_type))
          .reduce((a, b) => a + Number(b.amount), 0) +
        students
          .filter(
            (s) =>
              (cls === "Tất cả" || s.class_type === cls) &&
              s.start_date?.slice(0, 7) === key &&
              !payments.some((p) => p.student_id === s.id && p.month.slice(0, 7) === key) &&
              !entries.some(
                (e) =>
                  e.income_type === "hoc_phi" &&
                  e.student_name === s.name &&
                  (e.course_label ?? "") === `${coursePrefix(s.class_type)}${s.course_index ?? 1}`,
              ),
          )
        ...
        rows: [
          ...
          ...paidRows.map((p) => {
            ...
            return [
              s?.name ?? "",
              s?.class_type ?? "",
              s ? `${coursePrefix(s.class_type)}${s.course_index ?? 1}` : "",
              s ? `${fmtDate(s.start_date)} → ${fmtDate(s.end_date)}` : "",
              fmtDate(p.paid_date),
              Number(p.amount),
            ];
          }),
        ...
        <CardContent className="space-y-5">
          ...
          {view === "year" && (
            <div>
              ...
              {byMonth.length === 0 ? (
                <EmptyState text="Chưa có dữ liệu trong năm này." />
              ) : (
                    ...
                    <TableBody>
                      {byMonth.map((r) => (
                        <TableRow key={r.m}>
                          ...
                          <TableCell className="text-right text-[color:var(--success)]">
                            {formatMoney(r.thu)}đ
                          </TableCell>
          ...
          {rows.map((e) => (
            <TableRow key={e.id}>
              ...
              <TableCell className="font-medium">
                {e.category}
                {e.is_fixed && <span className="ml-1 rounded bg-muted px-1 text-[10px]">cố định</span>}
              </TableCell>
...
function DeleteEntryButton({ id }: { id: string }) {
  ...
  return (
    <Button
      ...
      onClick={() => confirm("Xóa khoản này?") && mut.mutate()}
    >
...
export function EntryDialog({
  ...
  const initKind: FormKind = existing
    ? existing.kind === "chi"
      ? "chi"
  ...
  const [amountStr, setAmountStr] = useState(
    existing ? formatMoney(Number(existing.unit_amount || existing.amount)) : "",
  );
  ...
  const expenseChips = useMemo(() => {
    ...
    return [
      ...cats.filter((c) => c.active),
      ...extra.map((n) => ({ id: n, name: n, default_amount: 0, sort_order: 99, active: true })),
    ];
  }, [cats]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] max-w-md overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{existing ? "Sửa khoản" : "Thêm khoản thu/chi"}</DialogTitle>
        </DialogHeader>
                    ...
                    {expenseChips.map((c) => (
                      <button
                        ...
                        onClick={() => {
                          setCategory(c.name);
                          setIsFixed(true);
                          if (Number(c.default_amount) > 0) setAmountStr(formatMoney(Number(c.default_amount)));
                        }}
                      >
                        {c.name}
                      </button>
            ...
            <div className="grid grid-cols-[1fr_100px] gap-3">
              ...
              <p className="col-span-2 text-xs text-muted-foreground">
                ...
                <span className="font-semibold">
                  {formatMoney(parseMoney(amountStr) * Math.max(1, Number(qty) || 0))}đ
                </span>
              </p>
            </div>
          )}
