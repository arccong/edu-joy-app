...
async function sendTelegram(text: string) {
  ...
  if (!res.ok || body?.ok === false) {
    throw new Error(`Telegram lỗi: ${body?.description ?? res.statusText}`);
  }
  return { ok: true };
}
...
export const sendTodayScheduleTelegram = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const { data: sched, error } = await sb.from("class_schedule").select("*").eq("day_of_week", dow).order("start_time");
  if (error) throw new Error(error.message);
  ...
  if (!sched || sched.length === 0) {
    ...
    for (const s of sched) {
      text += `🎵 <b>Lớp ${s.class_type}</b>\n⏰ ${String(s.start_time).slice(0, 5)} - ${String(s.end_time).slice(0, 5)}${s.location ? ` • 📍 ${s.location}` : ""}\n\n`;
    }
  }
